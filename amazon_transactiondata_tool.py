...
quantity_sales = df.loc[df.type == 'Order', ['store', 'type', 'sku', 'quantity']]
...
# 收入类-收到运费
income_shipping_credits = df.loc[:, ['store', 'type', 'sku', 'shipping_credits']]
income_shipping_credits.rename(columns={'shipping_credits': '运费'}, inplace=True)

# 收入类-销售收入
income_product_sales = df.loc[:, ['store', 'type', 'sku', 'product_sales']]
income_product_sales.rename(columns={'product_sales': '商品价格'}, inplace=True)

# 收入类-折扣:促销返点
income_promotional_rebates = df.loc[:, ['store', 'type', 'sku', 'promotional_rebates']]
income_promotional_rebates.rename(columns={'promotional_rebates': '促销返点'}, inplace=True)

# 收入类-折扣:销售税
income_sales_tax_collected = df.loc[:, ['store', 'type', 'sku', 'sales_tax_collected']]
income_sales_tax_collected.rename(columns={'sales_tax_collected': '销售税'}, inplace=True)

# 收入类-折扣:平台服务税
income_Marketplace_Facilitator_Tax = df.loc[:, ['store', 'type', 'sku', 'Marketplace_Facilitator_Tax']]
income_Marketplace_Facilitator_Tax.rename(columns={'Marketplace_Facilitator_Tax': '平台服务税'}, inplace=True)

# 店内费用类-佣金
sellingfees = df.loc[:, ['store', 'type', 'sku', 'selling_fees']]
sellingfees.rename(columns={'selling_fees': '佣金'}, inplace=True)

# 店内费用类-亚马逊物流基础服务费
fba = df.loc[:, ['store', 'type', 'sku', 'fba_fees']]
fba.rename(columns={'fba_fees': '亚马逊物流基础服务费'}, inplace=True)

# 店内费用类-广告费用
other_transaction = df.loc[:, ['store', 'type', 'sku', 'other_transaction_fees']]
other_transaction.rename(columns={'other_transaction_fees': '广告费用'}, inplace=True)

# 店内费用类-其他：订单
order_other = df.loc[df.type == 'Order', ['store', 'type', 'sku', 'other']]
order_other.rename(columns={'other': '其他_订单'}, inplace=True)

# 店内费用类-其他：服务费
service_fee_other = df.loc[df.type == 'Service Fee', ['store', 'type', 'sku', 'other']]
service_fee_other.rename(columns={'other': '其他：服务费'}, inplace=True)

# 店内费用类-其他：其他：退货
refund_other = df.loc[df.type == 'Refund', ['store', 'type', 'sku', 'other']]
refund_other.rename(columns={'other': '其他：退货'}, inplace=True)

# 店内费用类-其他：其他：清算
adjustment_other = df.loc[df.type == 'Adjustment', ['store', 'type', 'sku', 'other']]
adjustment_other.rename(columns={'other': '其他：清算'}, inplace=True)

# 店内费用类-其他：其他：闪电交易费
lightning_Deal_Fee_other = df.loc[df.type == 'Lightning Deal Fee', ['store', 'type', 'sku', 'other']]
lightning_Deal_Fee_other.rename(columns={'other': '其他：闪电交易费'}, inplace=True)

# 店内费用类-其他：其他：Debt
debt_other = df.loc[df.type == 'Debt', ['store', 'type', 'sku', 'other']]
debt_other.rename(columns={'other': '其他：Debt'}, inplace=True)

# 店内费用类-其他：其他：退款
chargeback_Refund_other = df.loc[df.type == 'Chargeback Refund', ['store', 'type', 'sku', 'other']]
chargeback_Refund_other.rename(columns={'other': '其他：退款'}, inplace=True)

# 店内费用类-其他：其他：FBA 库存费
FBA_Inventory_Fee_other = df.loc[df.type == 'FBA Inventory Fee', ['store', 'type', 'sku', 'other']]
FBA_Inventory_Fee_other.rename(columns={'other': '其他：FBA 库存费'}, inplace=True)

# 店内费用类-包装费
gift_wrap_credits_fees = df.loc[:, ['store', 'type', 'sku', 'gift_wrap_credits']]
gift_wrap_credits_fees.rename(columns={'gift_wrap_credits': '包装费'}, inplace=True)

# 其他：提款
tixian = df.loc[df.type == 'Transfer', ['store', 'type', 'sku', 'total']]
